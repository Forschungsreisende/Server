...
int ClientCount = 0; //Изначально нет подключаемых пользователей
...
int main()
{
	...
	Connections = (SOCKET*)calloc(64,sizeof(SOCKET)); //Выделяем память

	ZeroMemory(&hints,sizeof(hints));

	hints.ai_family = AF_INET; //Рассписываем сокет
	...
	for (;;Sleep(75))
	{
		if (Connect = accept(Listen,NULL,NULL)); //Подключается пользователь
		{
